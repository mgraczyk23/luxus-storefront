...
export default function AboutPage({
  ...
  const c = {
    ...
    stats: [
      [text.stat1Number ?? '', text.stat1Label ?? ''],
    ...
    excellenceItalic:  text.excellenceItalic  ?? '',

    storyHeading:   text.storyHeading   ?? '',
    storyPara1:     text.storyPara1     ?? '',
    storyPara2:     text.storyPara2     ?? '',
    storyPara3:     text.storyPara3     ?? '',
    storyPara4:     text.storyPara4     ?? '',
    storyPullquote: text.storyPullquote ?? '',
    storyParaFinal: text.storyParaFinal ?? '',

    phil1Title: text.phil1Title ?? '',
    phil1Body:  text.phil1Body  ?? '',
    phil2Title: text.phil2Title ?? '',
    phil2Body:  text.phil2Body  ?? '',
    phil3Title: text.phil3Title ?? '',
    phil3Body:  text.phil3Body  ?? '',

    missionHeading: text.missionHeading ?? '',
    missionBody1:   text.missionBody1   ?? '',
    missionBody2:   text.missionBody2   ?? '',
    missionCallout: text.missionCallout ?? '',
    pillars: [
      [text.pillar1Title ?? '', text.pillar1Body ?? ''],
      [text.pillar2Title ?? '', text.pillar2Body ?? ''],
      [text.pillar3Title ?? '', text.pillar3Body ?? ''],
      [text.pillar4Title ?? '', text.pillar4Body ?? ''],
    ] as [string, string][],

    curationHeading: text.curationHeading ?? '',
    curationIntro:   text.curationIntro   ?? '',
    criteria: [
      [text.crit1Title ?? '', text.crit1Body ?? ''],
      [text.crit2Title ?? '', text.crit2Body ?? ''],
      [text.crit3Title ?? '', text.crit3Body ?? ''],
      [text.crit4Title ?? '', text.crit4Body ?? ''],
    ] as [string, string][],

    fflBody:      text.fflBody      ?? '',
    fflCard1Body: text.fflCard1Body ?? '',
    fflCard2Body: text.fflCard2Body ?? '',
  }
        ...
        <div style={{ maxWidth:"1440px",margin:"0 auto" }}>
          ...
          <div className="lxs-about-brands">
            {(brands.length > 0 ? brands.map(b => ({
              ...
              { name:"Heckler & Koch",   origin:"Germany · Oberndorf",        description:"Relentless German engineering applied to every pistol they produce. The P7, HK45, Mark 23, and USP series occupy a singular position — iconic design married to mechanical excellence that has never been fully replicated.",                                          logo:null, slug:"heckler-koch" },
