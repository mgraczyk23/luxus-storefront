...
import type { SiteSettings, SupportPageText } from '@/lib/payload'
...
export default function SupportPage({ settings, text = {} }: { settings: SiteSettings; text?: SupportPageText }) {
  const { t } = useTheme()
  ...
  const c = {
    ...
    topics: (() => {
      ...
      const list = cms.length > 0 ? cms : ["Order Status", "Shipping Question", "Product Question", "FFL Transfer", "Account Issue", "Return / Exchange", "Other"]
  ...
  const [form, setForm] = useState({ firstName:"", lastName:"", email:"", phone:"", orderNumber:"", topic:TOPIC_PLACEHOLDER, message:"", fflConsent:false })
          ...
          <div>
            ...
            {formStatus === "success" ? (
              <div style={{ padding:"60px 40px",border:`1px solid ${t.border}`,background:"#fff",textAlign:"center" }}>
                ...
                <button onClick={()=>{ setFormStatus("idle"); setForm({ firstName:"",lastName:"",email:"",phone:"",orderNumber:"",topic:TOPIC_PLACEHOLDER,message:"",fflConsent:false }) }}
            ...
            <div>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:"10px" }}>
                {[
                  ...
                  { label:"Track Your Order", sub:"Get real-time tracking and shipment status", href:"/account", icon:<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><rect x="2" y="5" width="14" height="9" rx="1" stroke="currentColor" strokeWidth="1.2"/><path d="M2 8H16" stroke="currentColor" strokeWidth="1.2"/><circle cx="5.5" cy="11" r="0.7" fill="currentColor"/></svg> },
                  ...
                  { label:"Submit a Consignment", sub:"Start the process to consign a piece from your collection", href:"/sell-your-gun", icon:<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M3 4.5L9 1L15 4.5V11.5L9 15L3 11.5V4.5Z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round"/><path d="M9 8L9 15M9 8L3 4.5M9 8L15 4.5" stroke="currentColor" strokeWidth="1.2"/></svg> },
        ...
        <div style={{ maxWidth:"1440px",margin:"0 auto" }}>
          <div className="lxs-ffl-guide">
            ...
            <div style={{ display:"flex",flexDirection:"column" }}>
              {c.fflSteps.map(([title, desc], i, arr) => ({ n: String(i + 1).padStart(2, '0'), title, desc, last: i === arr.length - 1 })).map((step,i,arr) => (
                <div key={step.n} style={{ display:"flex",gap:"20px",alignItems:"flex-start",padding:"20px 0",borderBottom:step.last?'none':`1px solid ${t.border}` }}>
