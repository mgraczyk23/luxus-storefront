...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = seo.categories?.title || "Shop by Category"
  const description = seo.categories?.description || "Browse all categories at the Luxus Collection."
  return {
    title,
    description,
...
export default async function CategoriesDirectory() {
  const [categoriesRes, tileImagesRes, countRes] = await Promise.allSettled([
    getCategories(),
    getShopTileImages(),
    getProducts({ limit: "500", fields: "id,*categories,+metadata" }),
  ])
