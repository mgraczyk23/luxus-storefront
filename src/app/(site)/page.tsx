...
const PRODUCT_FIELDS = "id,title,handle,subtitle,thumbnail,*variants,*variants.prices,*variants.inventory_quantity,categories.id,categories.name,categories.handle,+metadata,*tags,*type,*attribute_values,*attribute_values.attribute_type"
...
export default async function Home() {
  ...
  if (featuredTag) {
    ...
    if (tagRes?.products?.length) {
      ...
      hasCuratedFeatured = true
    }
  }
  if (!hasCuratedFeatured) {
  ...
  // New arrivals = 4 most recent in-stock Collectible Firearms not already featured.
  // Use a dedicated tag-filtered query with a larger pool so we reliably surface 4,
  // even when the newest products overall are modern or out of stock. Falls back to
  // the homepage product set if the Collectibles tag isn't found.
  const featuredIds = new Set(featuredProducts.map(p => p.id))
  const collectiblesTag = allTags.find(t => t.value.toLowerCase() === 'collectibles firearms')
  let newArrivals = products
    .filter(p => !featuredIds.has(p.id) && p.in_stock && p.tags.includes('Collectibles Firearms'))
    .slice(0, 4)
  if (collectiblesTag) {
    const naRes = await getProducts({
      'tag_id[]': collectiblesTag.id,
      order: '-created_at',
      limit: '24',
      fields: PRODUCT_FIELDS,
    }).catch(() => null)
    if (naRes?.products?.length) {
      const pool = naRes.products
        .map(mapMedusaProduct)
        .filter(p => !p.is_backroom_hidden && p.in_stock && !featuredIds.has(p.id))
        .slice(0, 4)
      if (pool.length) newArrivals = pool
    }
  }
