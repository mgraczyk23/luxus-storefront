...
export async function POST(req: NextRequest) {
  ...
  const [salesRes, customerRes] = await Promise.all([
    sendEmail(EMAIL_SALES, `Wire/Check Order ${body.orderRef} — ${body.firstName} ${body.lastName} — ${fmt(body.amount)}`, salesHtml),
    sendEmail(body.email, `Order Received — Payment Instructions for ${body.orderRef}`, customerHtml),
  ])

  if (!salesRes.ok) {
    Sentry.captureException(new Error(`Wire checkout: sales email failed for order ${body.orderRef}`))
    return NextResponse.json({ error: 'Email send failed' }, { status: 502 })
  }
  if (!customerRes.ok) {
    Sentry.captureException(new Error(`Wire checkout: customer email failed for order ${body.orderRef} to ${body.email}`))
  }
