...
export default function ListingPage({
  ...
  const activePills = [
    ...(hideCategoryFilter ? [] : filters.categories.map(v => ({ key: 'categories', value: v, label: v }))),
    ...(hideBrandFilter    ? [] : filters.brand.map(v       => ({ key: 'brand',      value: v, label: v }))),
  ...
  const sidebarContent = (
    <div>
      ...
      {!hideCategoryFilter && uniqueCategories.length > 0 && (
      ...
      {!hideBrandFilter && uniqueBrands.length > 0 && (
        ...
        <div>
          ...
          {totalPages > 1 && (() => {
            ...
            <div className="lxs-pagination" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", marginTop: "28px", flexWrap: "wrap" }}>
              ...
              {isMobile ? (
                <span style={{ padding: "0 16px", fontSize: "13px", color: t.textMuted, fontFamily: "'Inter',sans-serif", letterSpacing: "0.04em", display: "flex", alignItems: "center" }}>
                  {page} / {totalPages}
                </span>
              ) : (
                getPageNums(page, totalPages, 4).map((n, idx) =>
                  n === '...' ? (
